\section{Introduction}
    ...
    With the rapid advancement of the capabilities of \edit{intelligent} computing technology to do tasks that were previously assumed to be too complicated for computers, there has been much recent discussion regarding how humans can trust this technology -- \edit{%if not always explicitly stated as such
    although the connection to trust is not always made explicit, per se}. 
...
\nisarcomm{never use first person voice --}    Specifically, this survey investigates what assurances an Artificially Intelligent Agent (AIA) can provide to a human user in order to affect their trust. The colloquial definitions of `appropriate use', `assurance', `AIA', and `trust' should suffice for now to give the reader a general idea of the motivation; more formal definitions will be presented in section \ref{sec:background}.
    ...
    \edit{To fully understand and appreciate the importance of assurances}, one must have a more formal understanding of each \edit{component} in figure \ref{fig:SimpleTrust_one_way}. %This is because the cycle is serial, and cannot be complete with any of the components missing. 
