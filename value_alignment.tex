...
\brettcomm{Formal, math explanation here}
The goal of value alignment can be thought of as trying to minimize the difference between the reward function $R_r$ of agent $r$ with that of a human (or humanity) $R_p$ ($p$ for people, $r$ for robot). \brettcomm{refer to delegation from miller, paragraph before 'user trust section'}

\brettcomm{english treatment here}
...
\brettcomm{Implications of solving that problem. What about solving these problems is an assurance?}
...
\paragraph{\brettcomm{need a different name, this descripbes a problem, not an assurance}Reward Hacking:}
